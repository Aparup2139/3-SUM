...
export const LeftContent = () => {
    ...
    const tasksData = sampleEvents.map((task, index) => ({ ...task, id: `task-${index + 1}`, isCompleted: Math.random() < 0.5 }));
    ...
    //     upcomingEvents = tasksData.filter(task => !task.isCompleted);
    //     pastEvents = tasksData.filter(task => task.isCompleted);
    // }
    ...
    return (<div className="h-full  w-full flex flex-col justify-center items-center " >
        ...
        <div className="flex h-full py-5 items-center w-full flex-col gap-5 overflow-y-scroll">
            {!isLoading && activeTab === 1 && upcomingEvents.map((task, i) => (
               <EventCard />
            ))}

            {!isLoading && activeTab === 1 && upcomingEvents.length === 0 && (
            ...
            {!isLoading && activeTab === 2 && pastEvents.length === 0 && (
                <div className="text-muted-foreground h-full text-sm flex flex-col gap-5 items-center justify-center">
            ...
            {isLoading && <EventCard loading={isLoading} />}
            {isLoading && <EventCard loading={isLoading} />}
            {!isLoading && activeTab === 2 && pastEvents.map((task, i) => (
                <EventCard
                    loading={isLoading}
                    key={task.id}
                    {...task}
